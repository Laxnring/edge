...
import '../sync/sync_policy.dart'
    show isLinkStale, ReconnectSupervisorAction, superviseReconnect;
...
class AppState extends ChangeNotifier {
  ...
  Future<void> _initSteps() async {
    ...
    if (isPaired) {
      if (_background) {
        ...
        if (!kIsWeb && Platform.isAndroid) EdgeTracking.start();
        ...
        try {
          ...
          if (await engine.connectToRemoteId(
            ...
            if (!kIsWeb && Platform.isIOS && !engine.liveEnabled) {
  ...
  Future<void> _provisionAdditionalAccessory() async {
    try {
      ...
      // provisional id.
      await LocalDb.upsertDevice(id: 'whoop:$remoteId', remoteId: remoteId);
    } catch (e) {
  ...
  void _maybeDowngradeLiveForBackground() {
    ...
    _bgLiveDowngrade = !kIsWeb && Platform.isAndroid
  ...
  Future<void> _armRecovery() async {
    if (kIsWeb || !Platform.isIOS || paired == null) return;
